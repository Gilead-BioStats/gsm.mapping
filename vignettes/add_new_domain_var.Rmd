...
1.  **Domain Name** (if applicable):
    ...
    -   If this domain has dependencies on other domains, please specify them. (e.g. the `COUNTRY` domain depends on `SUBJ` domain.) This will help aid the priority/order of in which the workflow will be ran.
2.  **Variable Name(s):**
...
3.  **Variable Metadata:**
    -   Provide detailed metadata for each variable, including:
        -   Expected Data Type (e.g., Character, Numeric, Date)
